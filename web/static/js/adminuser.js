...
function find() {
    ...
    $.ajax({
        ...
        dataType : "json",
        async:false,
        url : "../AdminUser?name="+str+"&status=1" +"&page=0"+"&limit="+limit,
            ...
            success:function(data){
                ...
                if (data.length>0) {
                    ...
                    for (var i = 0; i < showNum; i++) {
                        ...
                        html.push('<td>' + data[i].username + '</td>');
                        html.push('<td>' + data[ i].nickname + '</td>');
                        html.push('<td>' + data[ i].email + '</td>');
                        html.push('<td>' + data[i].ip + '</td>');
                        html.push('<td>' + data[i].login_time + '</td>');
                        html.push('<td>' + data[i].reg_time + '</td>');
                        // html.push('<td>' + data[i].status + '</td>');
                        html.push('<td>' + '<a href="Userinfo?uid='+data[i].uid+'"'+'><i class="fa fa-edit"></i></a>' + '&nbsp;&nbsp;&nbsp;&nbsp;'+'<a href="" data-toggle="modal" onclick="deletebook(' + data[i].uid + ')"><i class="fa fa-trash" style="color: red"></i></a>'+ '</td>');
...
function deletebook(uid) {
    con = confirm("是否删除?");
    if (con == true) {
        location.href = "/admin/DeleteUser?uid=" + uid;
    }
}
