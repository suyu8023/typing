...
function CheckInput(id) {
    ...
    var qaq = "qaq" + s;
    if (flag === 0) {
        qtime=document.getElementById("qtime").value*60;
        qtime1=qtime;
        // document.getElementById("time3").value=qtime*60;
        time1();
        countTime();
        speed1();
        flag = 1;
...
function countTime() {
    ...
    if (sss%5==0){
        data=data+"{y:"+document.getElementById("speed1").value+",x:"+sss+"},";
    }

    document.getElementById("times").innerHTML = (Array(2).join(0) + m).slice(-2) + ":" + (Array(2).join(0) + s).slice(-2);
    var mm =Math.floor(qtime1 / 60);
    var ss =Math.floor(qtime1 % 60);
    document.getElementById("times2").value = (Array(2).join(0) + mm).slice(-2) + ":" + (Array(2).join(0) + ss).slice(-2);
    if (leftTime==0){
        // var cid = document.getElementById("cid").value;
        data=data+']';
        document.getElementById("instan").value=data;
        document.getElementById("but").click();
        // location.href="/home/Contest/Result?cid="+cid;
    }
...
function speed1() {
    var s = (word1 - wrong1) / (qtime1-qtime) * 60;
