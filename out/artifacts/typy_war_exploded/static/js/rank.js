...
$(document).ready(function() {
    ...
    $.ajax({
        ...
        dataType : "json",
        async:false,
        url : "../home/Status?"+str+"&page=0" + "&limit="+limit+"&sta=1",
    ...
    function createTable(currPage, limit, total) {
        ...
        $.ajax({
            ...
            dataType : "json",
            async:false,
            url : "../home/Status?"+str+"&page="+currPage+"&limit="+limit+"&sta=1",
            success:function(data){
                // console.log(url);
                if (data.length>0) {
                    for (var i = 0; i < showNum; i++) {
                        ...
                        html.push('<td>' + '<a href="UserRank?uid='+data[i].uid+'"'+'>'+data[ i].username+'</a>' + '</td>');
                        html.push('<td>' + '<a href="UserRank?uid='+data[i].uid+'"'+'>'+data[ i].nickname+'</a>' + '</td>');
